...
def _convert_weight(model, model_key: str, gtensor, backend: str):
    ...
    if gtensor.ggml_type == "q6_k":
        # Genuine 6-bit packed weight (0.75 B/elem, symmetric, no zero) for the
        # W6A8 dp4a decode kernel. Built from the existing Q6_K int8 decode.
        from executorch.backends.cuda.packed_int6_tensor import CudaPackedInt6Tensor

        return CudaPackedInt6Tensor.from_intx_int8(
            gtensor.to_intx_unpacked_to_int8_tensor()
        )
